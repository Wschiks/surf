...
- **Not under the mist, though**: a zone whose area is not open yet (Sea/Ocean before the matching beach expansion) shows no badge at all - it is not really "in play", and a wall of lock badges across the misty part looked cluttered. A locked zone only gets a badge once its own area is already open (e.g. Level 2-4 of Wave surfing, and Skimboarding once the chain reaches it), same rule as `Haze`/`Mist` uses (`expansionNeededFor`).
...
## Save code option removed
- Removed the "Save code" row and page from the menu (back up/restore your game as a text code). Removed the underlying code too, not just the button: `exportSave`/`parseSave`/`writeSave` in `src/core/save.ts`, the whole `restored`/`onRestored` plumbing (`Menu` -> `GameUI` -> `MapScene`, which used to stop autosaving and reload the page after a restore), and the "save code" sentences in the Terms and the Privacy Policy (they promised a backup feature that no longer exists). The game still autosaves to the device as it always did (`saveGame`/`loadGame`, unaffected); there is just no way to export or import that save as text any more, and no way to move a save to another device or back it up outside the device.
- The menu's "Your save" section is gone with it (it only ever held this one row); Settings/Help/Legal keep their normal spacing.
- Tests: removed the `describe('save codes', ...)` block in `tests/save.test.ts` and the two `exportSave`-based assertions in `tests/shop.test.ts` (rewritten to check the same "perks and Club end date are not in the save" property via a plain `saveGame`/localStorage round trip instead). 114 tests now (was 116).

## The "next sport" tutorial step takes you there
- The **sports** coach mark (a whole new sport becomes affordable, e.g. Skimboarding once Level 2 of Wave surfing is owned and paid for) now **pans the camera to that sport's first zone** the moment it shows, before spotlighting the Sports button. You asked how a player is meant to know it exists at all, before finishing the sport they already have - now they see it appear on the map with its own lock badge, right as the hint darkens the screen, then get told to tap Sports to open it. Text updated: "You have enough to unlock Skimboarding! Look, it is right there. Tap Sports to open it."
- Reuses the existing camera-pan-to-a-zone behaviour (`onSelect`) that a normal zone tap already does; no new panning code.

## Three zone-sheet and skill-screen updates
- **Skill wheel only shows sports you have unlocked** (plus Beach, always free): the tabs row and the wheel's spokes/labels/nodes now filter to `t.id === 'beach' || state.sports[t.id]`. Locked sports keep their fixed ring slot (angles never change, so nothing else shifts when a new one appears) - they are just not drawn yet. Updates live while the screen is open, the moment a new sport is unlocked (`SkillScreen.sportsKey()` fingerprints which sports are unlocked; when it changes, the tabs and the wheel are rebuilt).
- **The zone sheet's second stat tile now shows the total for one whole session**, not the price per guest: label changed "each" -> "total", value changed `pricePerGuest` -> `income` (guests x price, exactly what one finished session pays). The "Level up" row's own before/after text still says "each" there on purpose, since that row is specifically about the per-guest price stat being upgraded.
- **Hire moves to the top of the upgrade list until you hire someone, then it settles back to the bottom.** Decided when the sheet is built (opening the zone), not while it stays open, as asked: hiring a manager mid-session does not reshuffle the list under your thumb, it is back at the bottom the next time you open that zone.

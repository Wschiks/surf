...
for (const s of SPORTS) console.log(`| ${s.id} | ${s.name} | ${s.icon} | ${s.area} | ${s.guestKind} | ${s.noun} | ${s.guestColors.join(' ')} | ${s.beachSite ? `${s.beachSite.id} (${s.beachSite.rect.x},${s.beachSite.rect.y},${s.beachSite.rect.w}x${s.beachSite.rect.h})` : '-'} | ${s.unlock ? `Level ${s.unlock.level} of ${s.unlock.after}, ${f(s.unlock.coins)} coins` : 'open from the start'} |`);
...
console.log('\n### Levels (zones)');
console.log('| zone | name | who | conditions | includes | rect x,y,w,h | tier | base guests | base sec | water look | unlock: coins / prev-level upgrades | base income/session | base coins/sec |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|');
for (const s of SPORTS)
  s.levels.forEach((l, i) => {
    const income = l.baseGuests * tierFactor(l.tier);
    console.log(`| ${s.id}-${i + 1} | ${l.name} | ${l.guests} | ${l.conditions} | ${l.starterBuys.join('; ')} | ${l.rect.x}, ${l.rect.y}, ${l.rect.w}, ${l.rect.h} | ${l.tier} | ${l.baseGuests} | ${l.baseSeconds} | ${l.look} | ${l.unlock ? `${f(l.unlock.coins)} / ${l.unlock.prevLevelUpgrades ?? '-'}` : 'free'} | ${f(income)} | ${f(income / l.baseSeconds)} |`);
  });
...
console.log('\n### Expansions\n| n | name | opens | level | coins | blurb |\n|---|---|---|---|---|---|---|');
for (const e of EXPANSIONS) console.log(`| ${e.n} | ${e.name} | ${e.opens} | ${e.level} | ${f(e.coins)} | ${e.blurb} |`);

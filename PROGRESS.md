...
First-run intro fixed: the "seen it" flag now lives outside the save (`src/core/introFlag.ts`) so "Start over" cannot bring the intro back for a returning player; the block is also lifted during idle "wait" stretches (nothing to tap yet) so the player can look around freely, and any sheet left open when the block returns is auto-closed first so the hole can never land behind it.

## Pre-launch pass
Publisher filled in (`src/config/legal.ts`: MugStudio, woutschiks@gmail.com; website left blank until mugstudio.nl is live) - `npm run store:check` now passes every computer-checkable item. Full manual click-through of every sheet, dialog and settings page (skills, beach, sports, expand incl. the real big-wave animation, shop incl. the demo ad and gem packs, hire-a-manager ordering, welcome-back boosts, quest claim, locked-level detail, Terms/Privacy/About with the new contact text) found and fixed one real cosmetic bug (the "Best value" shop tag could wrap mid-word, `src/styles.css`) and confirmed no dead ends. What is left before a store submission is entirely accounts, signing and store forms (see docs/STORE-READINESS.md) - nothing left in the app itself.

## Expansions no longer pay gems; economy document
Buying a beach expansion used to add 15 / 25 skill points; that is gone (author's request). Skill points now come from quests only. `EXPANSION_POINTS`, its toast text and the "Quests and expansions give skill points" subtitle were removed, tests updated (114 passing), design docs synced. New `docs/ECONOMY.md`, generated by `npm run docs:economy`: every upgrade formula, per sport and per zone, with numbers from the real code and the balance-bot timeline.
